...
class TestLoginPresenter {
    ...
    private val netConnection: NetContract = mockk()
    ...
    @BeforeEach
    fun setup() {
        clearMocks(authManager, view, netConnection)
        scheduler = TrampolineSchedulerProvider()
        loginPresenter = LoginPresenter(authManager, scheduler, netConnection)
        loginPresenter.onSetView(view)
    }

    @Nested
    inner class ConnectedToTheInternet {

        @BeforeEach
        fun `set internet connection to true`() {
            every { netConnection.hasInternetConnection() } returns Single.just(true)
        }
        ...
        inner class SignInBySnookie {
            ...
            fun `with really unknown error then update UI`() {
                val response = Single.just(
                    Result(
                        isSuccessful = false,
                        code = "random characters to simulate unknown result code"
                    )
                )
                every { authManager.signInBySnookie(EMAIL, PASSWD) } returns response
                every { view.unknownError() } just Runs
    ...
    @Nested
    inner class NotConnectedToTheInternet {

        @BeforeEach
        fun `set internet connection to false`() {
            every { netConnection.hasInternetConnection() } returns Single.just(false)
        }

        @Test
        fun `sign in anonymously then inform view about no internet connection`() {
            every { view.noInternetConnection() } just Runs
            ...
            verify { view.noInternetConnection() }
        }

        @Test
        fun `sign in by facebook then inform view about no internet connection`() {
            every { view.noInternetConnection() } just Runs
        ...
        @Test
        fun `sign in by snookie then inform view about no internet connection`() {
            every { view.noInternetConnection() } just Runs
